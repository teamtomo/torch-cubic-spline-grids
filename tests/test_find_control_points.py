...
def test_find_control_points():
    ...
    result = find_control_points_1d(sample_positions, torch.tensor([2.5]))
    expected = torch.tensor([[1, 2, 3, 4]])
    assert torch.allclose(result, expected)

    # sample on point should be included as lower bound in interval
    result = find_control_points_1d(sample_positions, torch.tensor([2]))
    expected = torch.tensor([[1, 2, 3, 4]])
    assert torch.allclose(result, expected)

    # check the same is true for 3, the upper bound of the same interval
    result = find_control_points_1d(sample_positions, torch.tensor([3]))
